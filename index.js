...
axios.interceptors.request.use((request) => {
  ...
  //sets cursor to 'loading progress'
  document.body.style.cursor = "progress"; //DEBUG!!!!!!
...
export async function favourite(imgId) {
  ...
  axios.get("https://api.thecatapi.com/v1/favourites").then((res) => {
    ...
    if (!deleted) {
      //else, we will post to the favourites
...
function getFavourites() {
  axios.get('https://api.thecatapi.com/v1/favourites')
    .then(res => {
      console.log('All Faves: ', res.data);
      Carousel.clear();

      res.data.forEach(item => {
        const element = Carousel.createCarouselItem(item.image.url, item.image.image_id, item.image.id)
